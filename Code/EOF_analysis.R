...
vast_wrapper <- function(n_x = 50){
  ## Seasonal model -----
  working_dir <- here::here("EOF_output/analysis/vast_seasonal_EOF3/")
  ...
  fit = fit_model(settings = settings,
                  ...
                  Lon_i = trait_dat$lon,
                  # t_i = trait_dat$year,
                  t_i = as.numeric(trait_dat$year_season) - 1,
                  ...
                  newtonsteps = 0,
                  covariate_data = data.frame(trait_dat, Lat=trait_dat$lat, Lon=trait_dat$lon, Year=as.numeric(trait_dat$year_season)-1),
                  X1_formula = ~ season,
...
vast_runs <- possibly_vast_wrapper(n_x = 200)
